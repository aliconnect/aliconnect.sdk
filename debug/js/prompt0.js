(function () {
  ...
  Aim.prompt = function prompt(selector, context) {
    ...
    // if (!is.elem) return;
    // if (context) {
    //   return prompts.set(selector, context);
    ...
    //   });
    // } else {
    //   console.log(selector, prompts.get(selector));
    // }
    ...
    if (prompts[selector]) {
      ...
      console.log('prompt', selector, prompt);
      const childPanels = [...is.elem.children].filter(elem => elem.tagName === 'DIV');
      const index = promptElem ? [...is.elem.children].indexOf(promptElem.elem) : 0;
      // childPanels.filter(elem => elem !== promptElement && elem.innerText).forEach(elem => setTimeout(() => elem.innerText = '', 1500));
      // return;
      childPanels.forEach(
        (elem, i) => $(elem).attr('pos', i < index ? 'l' : 'r')
      );
