...
class Text_Extraction
{
  ...
  * @return mixed|string
  */
  public static function doc_to_text( $path_to_file )
  {
    $line = file_get_contents($path_to_file);
    $line = explode(chr(18).chr(1).chr(0),$line);
    $line = $line[1];
    $line = explode(chr(0).chr(3),$line);
    $line = array_shift($line);
    return preg_replace('/[^a-zA-Z0-9\s\,\.\-\n\r\t@\/\_\(\)]/', '', $line);
  }

  // public static function doc_to_text1( $path_to_file )
  // {
  //     // $fileHandle = fopen($path_to_file, 'r');
  //     // $line       = @fread($fileHandle, filesize($path_to_file));
  //     $line       = file_get_contents($path_to_file);
  //     // $lines      = explode(chr(0x0D), $line);
  //     // $line = explode(chr(18).chr(1),$line); $line = array_pop($line);
  //     // $line = explode(chr(0).chr(3),$line);
  //     // $line = array_shift($line);
  //     // exit($line);
  //
  ...
  //       // if ($o<=6) continue;
  //       // if ($o>254) continue;
  //       // echo "{$i} [".ord($a[$i-4])."][".ord($a[$i-3])."][".ord($a[$i-2])."][".ord($a[$i-1])."] [{$o}] '{$c}'".PHP_EOL;
  //       echo "{$c} {$i} [".ord($a[$i-1])."][".ord($a[$i])."]".PHP_EOL;
  //     }
  //     $res = preg_replace('/[^a-zA-Z0-9\s\,\.\-\n\r\t@\/\_\(\)]/', '', $res);
  //     exit($res);
  //
  //
  //     $count = count($a);
  //     $r = '';
  //     for ($i = 0; $i <= $count; $i+=2) {
  //       $n = ord($a[$i+1])*256+ord($a[$i]);
  //       if ($n) $r .= chr($n);
  //       // if ($n) echo $n.' '.chr($n).PHP_EOL;
  //       // $o = ord($a[$i]);
  //       // if ($o<=6) continue;
  //       // if ($o>254) continue;
  //       // echo "{$i} [".ord($a[$i-4])."][".ord($a[$i-3])."][".ord($a[$i-2])."][".ord($a[$i-1])."] [{$o}] '{$c}'".PHP_EOL;
  //       // echo "{$i} ".$a[$i].$a[$i+1]." [".$n."] [".ord($a[$i])."][".ord($a[$i+1])."]".PHP_EOL;
  //       // echo $c;
  //     }
  //     // die($line);
  //     exit($r);
  ...
  //
  //
  //     $line = explode(chr(18).chr(1),$line);
  //     $line = array_pop($line);
  //     $line = explode(chr(82).chr(8),$line);
  //     $line = array_shift($line);
  //     $a = str_split($line);
  //     $count = count($a);
  //     $r = '';
  ...
  * @return bool|string
  */
  public static function docx_to_text( $path_to_file )
  {
    $response = '';
