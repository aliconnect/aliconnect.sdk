(function (){
  ...
  const libraries = {
    async page() {
      ...
      console.log($().storage('.tv.width'));
      $(document.documentElement).class(getItem('isApp') || 'page').attr('dark', localStorage.getItem('dark'));
      $(document.body).attr('host', new URL(document.location).hostname.split('.').slice(-2, -1)[0]).append(
        $('nav').append($('article').append(
          ...
          $('span').class('domainname').text(aim.displayName(new URL(document.location).hostname.split('.')[0])),
        ...
        $('main').append($('article').append(
          ...
          .css('width', $().storage('.tv.width') || '200px')
          .on('click', e => {
      ...
      if (aimClient.store('aimUser')) {
        ...
        $('.account span.user').text(user.name || user.accountname || '');
        $('.account>nav').append(
          $('button').caption('gegevens').on('click', e => {
            dmsClient.api('/me').select('*').get().then(row => {
              row.schemaName = 'Contact';
              pageviewrow(row);
            });
          }),
          $('button').class('icn-logout').caption('afmelden').on('click', signOut),
        );
      } else {
        $('.account>nav').append(
          $('button').class('icn-login').caption('aanmelden').on('click', signIn),
        );
      }
    ...
    async forms(){
        ...
        (function buildForm(properties, data) {
          for (let [propertyName,property] of Object.entries(properties)) {
            ...
            if (property.properties) {
              ...
              $('details').parent(elem)
              .open(localStorage.getItem(propertyName + 'Open'))
              .on('toggle', e => localStorage.setItem(propertyName + 'Open', e.target.open ? 1 : ''))
              .append($('summary').text(property.title || propertyName))
              .buildForm(property.properties, data ? data[propertyName] : null);
            } else {
              $('div').class('attr').parent(elem).append(
                property.description ? $('i').title(property.description) : null,
                $('label').class('title').text(property.title || nameToTitle(propertyName)),
      ...
      let formDefinitions = {
        ...
        client: {
          ...
          properties: {
            client_id: {
              placeholder: 'Voer uw client_id in',
              // required: true,
            },
            client_secret: {
              placeholder: 'Voer uw client_secret in',
            },
          },
        },
      };
      ...
      // data = await aim.fetch('https://dms.aliconnect.nl/api/v1/app/config_data').query({
      //   path: new URL(document.location.href).pathname,
      // }).get();
      ...
      async function signDocument(name) {
        const body = replaceFields(await aim.fetch(docBasePath + name + '.md').get(),data);
        const elem = $('div').parent('.lv').class('col').append(
          $('nav').append(
            $('a').class('icn-back').caption('Terug').on('click', e => elem.remove()),
            $('button').class('icn-save').style('margin-left:auto;').caption('Opslaan').on('click', async e => {
              const canvas = document.querySelector("canvas");
              var image = new Image();
              image.src = canvas.toDataURL();
              canvas.parentElement.insertBefore(image, canvas);
              canvas.remove();
              aim.fetch('https://dms.aliconnect.nl/api/v1/app/config_data').query({
                path: new URL(document.location.href).pathname,
                client_id: data.client.client_id,
                savepdf: name,
              }).body($('.pv').elem.innerHTML).post().then(e => elem.remove());
            }),
          ),
          $('div').class('col pv').html(aim.markdown().render(body)).query('canvas', elem => elem.paint()),
        );
      }
      ...
      function start(){
        ...
        // var contentElem = formElem;
        inputId=0;
        const elem = $('form').parent('.lv').class('col pv').autocomplete("off").on('submit', async e => {
          e.preventDefault();
          console.log(data);
          sessionStorage.setItem('client_id', data.client.client_id);
          sessionStorage.setItem('client_secret', data.client.client_secret);
          ...
          // console.log(data);
          // return false;
          [data,formDefinitions] = await aim.fetch('https://dms.aliconnect.nl/api/v1/app/config_data').query({
            path: new URL(document.location.href).pathname,
            client_id: data.client.client_id,
            client_secret: data.client.client_secret,
            submitter: submit
          }).body(data).post();
          console.log(data,formDefinitions);
          // if (submit === 'next') {
          //   await signDocument('Explore-Legal-Verwerkers-overeenkomst');
          //   await signDocument('Explore-Legal-Protocol-meldplicht-datalekken');
          //   await signDocument('Explore-Legal-Verwerkingsregister');
          // }
          $('.pv .col').text('').buildForm(formDefinitions,data,true);
          // start();
          ...
          $('nav').append(
            $('button').class('icn-back').caption('Terug').on('click', e => elem.remove()),
            $('button').class('icn-save').style('margin-left:auto;').caption('Opslaan').value('save').default(true),
            // $('button').value('next').text('Verder'),
          ),
          $('div').class('col').buildForm(formDefinitions,data,true),
          // $('div').class('col pv'),
          // .append(
          //     $('nav').append(
          //     )
      ...
      // start();
      $(document.body).style('margin:auto;max-width:800px;').class('lv').append(
        $('div').class('col').append(
          $('nav').class('col').append(
            $('div').class('icn-config').text('Config formulier').on('click', start),
            $('div').class('icn-doc').text('Verwerkers overeenkomst').on('click', e => signDocument('Explore-Legal-Verwerkers-overeenkomst')),
            $('div').class('icn-doc').text('Protocol meldplicht datalekken').on('click', e => signDocument('Explore-Legal-Protocol-meldplicht-datalekken')),
            $('div').class('icn-doc').text('Verwerkingsregister').on('click', e => signDocument('Explore-Legal-Verwerkingsregister')),
          )
        )
      );
    },
  ...
  Elem.prototype = {
    ...
    paint(options) {
      console.log('PAINT',this.elem,options);
  ...
  Object.defineProperties(Elem.prototype, {
    ...
    seperator: { value: function seperator (selector, factor = 1) {
      const zindex = 6;
      const self = this;
      const selfElement = self.elem;
      let moveX;
      function start(e) {
        if (e.which === 1) {
          if (!e) e = window.event;
          ...
          selfElement.clientX = e.clientX;
          self.css('left', selfElement.moveX = 0).css('z-index', 300).attr('active', '');
      ...
      function doresizeelement(e) {
        self.css('left', (moveX = e.clientX - selfElement.clientX) + 'px');
      };
      function checkmouseup (e) {
        ...
        const selectorElem = aim(selector);
        selectorElem.css('width', (selectorElem.offsetWidth + factor * moveX) + 'px');
        console.log(selector + '.width', selectorElem.width);
        aim().storage(selector + '.width', selectorElem.width);
        self.css('left', moveX = 0).css('z-index', zindex).attr('active', null);
      };
      self
      // .attr(pos, '')
  ...
  'default,autoplay'.split(',').forEach(name => Object.defineProperty(Elem.prototype, name, {
    enumerable: false,
    value: function attr() {
      return this.attr(name, '');
    }
  }));
  'offsetWidth'.split(',').forEach(name => Object.defineProperty(Elem.prototype, name, {
    enumerable: false,
    get: function attr() {
      return this.elem[name];
    }
  }));
  'maxWidth,width'.split(',').forEach(name => Object.defineProperty(Elem.prototype, name, {
    enumerable: false,
    get: function get() { return this.elem.style[name]; },
    set: function set(value) { this.elem.style[name] = value; },
  }));

  Object.defineProperties(Elem.prototype, {
    query:{ value(selector, fn){ fn( aim(this.elem.querySelector(selector))); return this; }},
    // query:{ value(selector, fn){ fn( this.elem ); return this; }},
    querySelector:{value(){return aim(this.elem.querySelector(...arguments))}},
  ...
  // 'style'.split(',').forEach(name => Object.defineProperty(Elem.prototype, name, {
  //   enumerable: false,
  //   get: function get() { return this.elem[name]; },
  //   set: function set(value) { this.attr(name, value); },
  // }));
  ...
  // }
  //
  const APerson = function APerson () {
    console.log(1);
  };

  // APerson.prototype = personPrototype;
  Object.defineProperties(APerson, {
    assign: { value: function assign() { } },
  });
  Object.defineProperties(APerson.prototype, {
    constructor: { value: APerson },
    greet: { value: function greet() { console.log(`hello`); } },
  });
