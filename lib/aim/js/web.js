(function () {
  ...
  function search(s, callback) {
    ...
    Web.url.searchParams.set('$search', s);
    ...
    */
    ...
    print(success) {
      ...
      function done() {
        ...
        setTimeout(() => loaded ? null : print(), 10);
      }
    ...
    property (property, row, editable, parentProperty, parentProperties) {
      ...
      let onchange = event => {
        ...
        if (parentProperty && parentProperty.property.type === 'string') {
          console.debug('MAKE PARENT STRING');
      ...
      const types = {
        ...
        select: {
          ...
          edit: () => {
            ...
            let {schemaName} = property;
            return $('select').append(
              options.map(option => $('option').attr(option).text(option.caption).selected(option.value === value)),
      ...
      if (elem) {
        return this.class('attr',format,type,name).append(
          icn || description ? $('i').class(`icn-${icn || 'info'}`).title((description||'').replace(/\.\s/,'.\n')) : null,
    ...
    setProperty(property, item) {
      ...
      this.refresh = function () {
        ...
        if (min || max) {
          this.css('--value', (( Number(value) - min) / (max - min) * 100) +'%');
        }
        this.value(value || 0);
        // const {el} = this;
        // if (el.tagName === 'METER') {
  ...
  Object.assign(Listview, {
    ...
    prototype: {
      ...
      render(rows, options = {}) {
        ...
        // $('div.tv').attr('hide',1);
        rows = this.rows = (rows ? rows.value || rows.rows || rows : this.rows).map(row => new Item({schemaName},row));
