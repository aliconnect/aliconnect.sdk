    ...
    */
    ...
    property (property, row, editable, parentProperty, parentProperties) {
      ...
      const types = {
        ...
        images: {
          view: () => $('div').class('images').append(
            value.map(row => $('img').src(row.src)),
          ),
          edit: () => {
          },
        },
        ...
        properties: {
          view: () => {
            // const properties = row.data[property.name];
            // console.debug('property', property, row.data[property.name]);
            // console.log({properties});
            return $('table').class('properties').append(
              Object.entries(value).map(([name,value]) => $('tr').append(
                $('th').text(name),
                $('td').text(value),
              ))
            )
