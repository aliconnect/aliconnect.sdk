(function(global, factory) { typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define(factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.Aim = factory()); }(this, function (exports) {
  ...
  Object.assign(Item, {
    prototype: {
      ...
      getAttribute (name) {
        ...
        const {type,properties} = this.properties[name] || {};
  ...
  Object.assign(Aim, {
    ...
    socketCall(options, value) {
      // if (!options || !options.address) return;
      ...
      // console.debug('socketCall', Aim.socketBusy, options, value);
      if (options) {
        // if (!options.address) throw new Error('Modbus with No address');
        return new Promise((succes,fail) => {
          socketBuffer.push([succes,fail,options,value]);
          Aim.socketCall();
        });
        ...
        if (socketAction) {
          ...
          var {name,fc,address,port,host,path,baudRate,register} = options;
          const baudRates = [2400,4800,9600,19200,38400,57600,115200];
          baudRate = baudRates[baudRate]||baudRate;
          // console.debug('socketAction',socketBuffer.length,port,host,path,baudRate,address,fc,register,name,value);
          fc = fc || 3;
          ...
          if (path) {
            ...
            const socket = new SerialPort({path,baudRate});
            socket.on('close', () => setTimeout(e => Aim.socketCall(Aim.socketBusy = false), 20));
            socket.on('open', async e => {
              ...
              function endRead(res){
                ...
                succes(Number(value));
                socket.close();
                // setTimeout(e => Aim.socketCall(Aim.socketBusy = false), 20);
              ...
              function endError(err){
                fail(err);
                socket.close();
                // console.error('endError DELAY');
                // setTimeout(e => Aim.socketCall(Aim.socketBusy = false), 500);
              }
              switch (fc) {
                // Coils
                case 1: {
                ...
                case 6: {
                  return await client.readHoldingRegisters(register, 1).then(endRead, endError);
                }
                // Multiple Coils
                case 15: {
    ...
    systemBuild(options={}) {
      ...
      const {socketClient, webServer} = options;
      // const {item, definitions, socketClient, webServer} = options;
      const globalProperties = [];
      const globalSystems = Aim.globalSystems = Aim.globalSystems || [];
      Object.values(definitions).forEach(item => Object.entries(item.properties||{}).forEach(([name,property]) => Object.assign(property,{name})));
      ...
      if (document) {
        ...
        globalSystems.forEach(item => {
          ...
          Object.entries(properties).filter(([name,property]) => !item.hasOwnProperty(name)).forEach(([name,property]) => {
            ...
            let to;
                        // Object.defineProperty(item, name, readOnly ? {get} : {get,set});
            Object.defineProperty(item, name, {
              ...
              set(value) {
                // console.debug('set', id, name, data[name], value)
                if (data[name] != value) {
                  // clearTimeout(to);
                  data[name] = value;
                  // to = setTimeout(e => {
        ...
        items.forEach((item,i) => {
          ...
          propertiesArray.forEach(([name,property]) => {
            const {readOnly,modbus,fc,register,readInterval} = property;
            ...
            Object.defineProperty(item, name, {
              ...
              set(value){
                if (data[name] != value) {
                  console.debug('set', name, value, {fc, register});
                  data[name] = isNaN(value) ? value : Number(value);
        ...
        Aim.on('message', async (event) => {
          ...
          switch (message_type) {
            ...
            case 'CHANGE': {
              ...
              // console.debug({id,name,value},items.filter(item => item.id == id));
