Abis = Object.setPrototypeOf({
  async init(content) {
    ...
    config({
      scope: 'openid profile name email medewerker admin',
      info: {
      },
      nav: {
        search: false,
    ...
    config(content);
    var {clientId,serviceRoot,socketRoot,authRoot,configfiles} = config;

    for (let src of configfiles || []) {
      console.log(src);
      await Aim.fetch(src).get().then(config);
    }
    ...
    var {clientId,client_id,info,definitions,options,tags,scope,nav} = config;
    ...
    $(document.body).class('col').attr('host', url.hostname.split('.').slice(-2, -1)[0]).append(
      $('nav').append(
        $('div').class('mw').append(
          ...
          info.title || info.description ? $('span').class('domainname').text(Format.displayName(info.title)).title(info.description) : null,

          !nav.search ? null : $('form').class('search row aco')
          .on('submit', (event) => {
            event.preventDefault();
          ...
          !nav.lang ? null : $('button').class('icn-local_language').on('click', (event) => event.elem = $('nav').parent(event.target).append(
          ...
          !nav.dark ? null : $('button').class('icn-dark_theme').on('click', (event) => $(document.documentElement).class('dark', Web.getItem('dark',dark^=1))),
          !nav.shop ? null : $('button').class('icn-cart').append(
            $('nav').append(
          ...
          !nav.chat ? null : $('button').class('icn-chat_multiple').on('click', e => Prompt.open('chat')),
          ...
          !config.settings ? null : $('button').class('icn-settings').on('click', e => Prompt.open('config')),
          !config.help ? null : $('button').class('icn-question').on('click', e => Prompt.open('help')),
          !config.account ? null : $('button').class('icn-person account').caption(account.id.name).on('click', e => Prompt.open('account')),
          // .append(
          //   $('div').append(
