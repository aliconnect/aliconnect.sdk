...
(function(global, factory) {
  ...
  function swipedetect(el, callback) {
    ...
    distX,
    distY,
    threshold = 150,     restraint = 100,     allowedTime = 300,     elapsedTime,
    startTime,
      ...
      const options = {
        ...
        e.on('data', function (data){
          ...
          try {
            ...
            : e.target.responseText;
            e.response = e.body = e.body;           } catch(err){
  ...
  aim.prototype = {
    ...
    analytics() {
      (function (i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function (){(i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)})(self,document,'script','https:      ga('create', {
        ...
        show(){
          this.external('show', null, par => {
            console.debug('SHOW REPLY', par);
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                filedownload(par){
          this.external('filedownload', "http:            console.debug('SHOW REPLY', par);
          });
        },
        mailimport(){
          this.external('show', null, par => {
        ...
        async load() {
          if (!localStorage.getItem('cookieSettings')) {
            ...
            .append(
              ...
              Aim('a').text('Cookie beleid').href('#?l=            )
          }
          return this;
        }
      })
  ...
  Object.defineProperties(Aim.prototype, {
      ...
      const documentUrl = new URL(document.location);
        ...
        if (this.execUrl(url.hash.substr(1))) {
          ...
          return;
            ...
            if (url.searchParams.get('l')) {        documentUrl.searchParams.set('l', url.searchParams.get('l'));
    ...
    id: {value: function (selector){
      this.key = selector;       this.set('id', selector);
    ...
    notify: {value: function (title, options) {
      ...
      if ("Notification" in self) {
        if (Notification.permission === "granted") {
                                                                                                                                                                                                                                                                                                                                                                        options = {
            body: `Bla Bla`,
            url: 'https:            icon: 'https:            image: 'https:                        data: {
  ...
  Object.defineProperties(Application.prototype, {
        ...
        if (options){
          Aim().url(AUTHORIZATION_TOKEN_URL).post(Object.assign({
            grant_type: 'authorization_code',
                        ...
                        access_type: 'offline',           }, options)).then(e => {
    ...
    refreshToken:{value:function(){
      ...
      if (this.refreshTokenHandle) return;
            this.refreshTokenHandle = new Aim.Client('https:        grant_type: 'refresh_token',
          ...
          self.addEventListener("message", (event) => {
            ...
            if (event.data.url) {
              ...
              if (url.searchParams.has('token')) {
                ...
                Aim().url('https:                .query('client_id', this.config.auth.client_id)
  ...
  Object.defineProperties(Client.prototype, {
    ...
    createLoginFrame: { value: function (params){
      params = Object.assign(params, {
        ...
        scope: 'name email',
                                                      ...
                                                      let url = 'https:      console.debug('LOGIN', url);
    ...
    publish: { value: function (){
      ...
      new Aim.HttpRequest(Aim.config.Aim, 'POST', '/', aimapi, e => {
        ...
        return;
        var swaggerUrl = 'https:        var onlineUrl = 'https:        console.debug(swaggerUrl, onlineUrl);
      ...
      for (var SchemaName in api.components.schemas){
        ...
        api.paths['/' + schemaName] = api.paths['/' + schemaName] || {
          get: {
            ...
            parameters: def.paths.item.get.parameters,             responses: def.paths.item.get.responses,             security: schema.security.get,
          },
          post: {
            ...
            parameters: def.paths.item.get.parameters,             responses: def.paths.item.get.responses,             requestBody: {
        ...
        api.paths['/' + schemaName + "(id)"] = api.paths['/' + schemaName + "(id)"] || {
          get: {
            ...
            parameters: def.paths.item.get.parameters,             responses: {
          ...
          patch: {
            ...
            parameters: def.paths.item.get.parameters,             responses: def.paths.item.get.responses,             security: schema.security.patch,
          },
          delete: {
                        tags: [schemaName],
            Summary: "Deletes a " + SchemaName,
            operationId: "item('" + SchemaName + "').delete",
            parameters: def.paths.item.get.parameters,             responses: def.paths.item.get.responses,             security: schema.security.delete,
          }
        }
                                                                              }
    ...
    qrcode: { value: function (selector){
      ...
      new QRCode(selector, {
                text: Aim.config.Aim.websocket.socket_id ? 'https:        width: 160,
        height: 160
      });
    }},
  ...
  Object.defineProperties(Item.prototype, {
        ...
        return parentElement.createElement('DIV', 'pricerow col', [
          ...
          ['DIV', 'shopbag', [
            ['INPUT', 'addbag', {type:'number', value:this.amount = product ? product.Data : '', onchange: (e)=>{
              return               Aim.shop.add(this.row, e.target.value);
            }}],
    ...
    isInherited: {
      get(){
                return this.getValue('InheritedID') ? 1 : 0;      },
    },
    isDraft: {
      get(){
      ...
      value: function (){
        ...
        if (!attributes[attributeName]) attributes[attributeName] = {
          value: this.data[attributeName].value, displayvalue: this.properties[attributeName].displayvalue
                        ...
                        var displayvalue = attributes[attributeName].displayvalue, value = attributes[attributeName].value;                        displayvalue = String(displayvalue).split('-').length == 3 && String(displayvalue).split(':').length == 3 && new Date(displayvalue) !== "Invalid Date" && !isNaN(new Date(displayvalue)) ? new Date(displayvalue).toISOString().substr(0, 19).replace(/T/, ' ') : displayvalue;
    ...
    submit: {
      value: function (e) {
        ...
        return;
          ...
          item.userlist = {};
          for (var i = 0, e, c = this.elUsers.getElementsByTagName('A') ; e = c[i]; i++) if (e.id) users.push(item.userlist[e.innerText] = e.id);        }
                Aim.ws.request({
          to: [ { sub: Aim.auth.sub } ],
          showNotification: [this.Title, {
                        tag: this.ID,
            body: 'Modified',             click_action: document.location.href,
  ...
  function Server(config) {
    ...
    function processRequest (req, res) {
      ...
      if (url.pathname === '/sql.json') {
        ...
        return new mssql.Request().query(sql, (err, res) => {
          if (err)           end(200, { 'Content-Type': 'application/json' }, JSON.stringify(res.recordsets));
        })
      }
          ...
          if (data.headers) {
            ...
            if (accessToken) {
              ...
              try {
                ...
                Aim().url('https:                  request_type:'check_access_token',
                  headers: {
    ...
    const options = config.http.ca ? {
      ...
      ca: config.http.ca ? fs.readFileSync(process.cwd() + config.http.ca) : null,
        ...
        const http = require(protocol);
    const server = http.createServer(options, processRequest).listen(config.http.port);
    const ws = require('ws');
    const wsServer = new ws.Server({server}).on('connection', onconnection);
    ...
    if (config.dbs) {
      this.sql.connect(config.dbs).then(a => {
        console.log('SQL ONN');
        this.sql.query(
  ...
  Object.defineProperties(aim, {
    ...
    config: { value: {
      ...
      trackLocalSessionTime: 5000,       trackSessionTime: 30000,       debug: 1,
      minMs: 60000,
      auth: {
    ...
    const: { value: {
      ...
      return Aim.promise( 'maps', resolve => {
        ...
        .attr('src', 'https:        .on('load', e => resolve (self.google.maps))
      });
    },},
    object: { value: {
      isFile(ofile) {
