...
(function (){
  ...
  function Om() {
    ...
    function Treeview(selector) {
      console.log('Treeview', selector);
                  this.elem = tree().id('tree').css('max-width', $().storage('tree.width') || '200px'),
      this.construct(...arguments);
      this.class('col aco atv noselect np')
    ...
    $().on('load', e => {
        ...
        $('section').id('om-main').append(
          om.tree = $('section').id('tree').css('max-width', $().storage('tree.width') || '200px'),
          $('div').seperator(),
  ...
  if (this.document) {
    ...
    const apiorigin = $.httpHost === 'localhost' && $().storage === 'api' ? 'http:        require = function () {};
